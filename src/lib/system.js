/**
 * =========================================
 * 📌 FILE: src/lib/system.js (RECONSTRUCTED)
 * 📌 DESCRIPTION:
 * Case management system untuk bot WhatsApp
 * Mengelola CRUD operations untuk case commands
 *
 * 📁 MAPPING: Library/system.js (original) → src/lib/system.js
 * =========================================
 */
...
const CASE_FILE = path.join(process.cwd(), "./src/core/WhosTANG.js");
...
const Case = {
     ...
     * @param {string} n - Nama case yang dicari
     * @returns {string} - Isi case lengkap
     */
    get: (n) => {
     ...
     * @param {string} cod - Kode case lengkap
     */
    add: (cod) => {
        let c = fs.readFileSync(CASE_FILE, "utf8");
     ...
     * @param {string} ky - Nama case yang akan dihapus
     */
    delete: (ky) => {
        let c = fs.readFileSync(CASE_FILE, "utf8");
